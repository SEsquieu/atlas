...
npm run demo:phone-voice-loop -- --dry-run
...
`demo:phone-voice-loop` is the hands-free proof command. It runs `loop:android voice-demo`: listens once through the Android node's native `audio.transcribe.once`, routes the transcript into `atlas session transcript`, refreshes visual context if the core freshness policy requires it, and speaks the Atlas answer back through native Android TTS. Use `--dry-run` to print the plan without touching the microphone or camera. Live prerequisites are the same as the text phone loop: Android/OpenClaw app foregrounded, paired/connected, microphone/camera permissions granted, and gateway allowlist containing `camera.snap`, `audio.speak`, `audio.stop`, and `audio.transcribe.once`.
...
```bash
npm run validate:ambient -- --store .atlas-runs/latest-ambient-android --require-capture --require-cached-ask --max-ask-refreshes 0
```

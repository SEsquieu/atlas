...
Android database version 3 creates a personal workspace automatically and scopes every new session, observation, and event to it. The same persistence boundary supports optional site, station, actor, task-run, procedure-revision, policy, and scoped-memory identities without placing enterprise administration in the consumer UI.
...
- `X-Atlas-Request-Id` supports cross-system tracing; and
- UUID-backed managed sessions forward organization scope for server-side authorization plus session/task-run correlation for cost attribution.

...
class OpenAiCompatibleBackend(
    ...
    override suspend fun infer(endpoint: ProviderEndpoint, apiKey: String?, request: InferenceRequest): InferenceResponse = withContext(Dispatchers.IO) {
        ...
        val httpRequest = Request.Builder()
            ...
            .apply { if (UUID_PATTERN.matches(request.sessionId)) header("X-Atlas-Session-Id", request.sessionId) }
            .apply { if (UUID_PATTERN.matches(request.workspaceId)) header("X-Atlas-Organization-Id", request.workspaceId) }
            .apply { request.taskRunId?.let { header("X-Atlas-Task-Run-Id", it) } }

...
const baseIntent: RoutingIntent = { capability: "fast", risk: "normal", latencyClass: "interactive", mediaPurpose: "standard_vision", hasImage: false };

function model(id: string, values: Partial<ModelCatalogEntry> = {}): ModelCatalogEntry {
  return {
    id, enabled: true, supportsVision: true, supportsTools: true,
    reasoningEfforts: ["none", "low", "medium", "high"],
    quality: { fast: 0.8, vision: 0.8, reasoning: 0.8, fallback: 0.8 },
    speed: 0.8, economy: 0.8, inputCreditsPerMillion: 1, outputCreditsPerMillion: 5, ...values,
  };
}
...
test("latency-critical work favors the measured faster model", () => {
  ...
  assert.equal(result.model.id, "quick"); assert.equal(result.reasoningEffort, "none"); assert.equal(result.maxOutputTokens, 160);
});
...
test("reasoning and safety-critical work favors measured quality", () => {
  ...
  assert.equal(result.model.id, "deep"); assert.equal(result.reasoningEffort, "high"); assert.equal(result.maxOutputTokens, 5_000);
});
...
test("hard constraints reject models without the required modality or effort", () => {
  const result = selectModel({ ...baseIntent, capability: "vision", risk: "elevated", hasImage: true }, [
    ...
    model("no-medium", { reasoningEfforts: ["none", "low"], speed: 1 }), model("eligible"),
  ]);
  assert.equal(result.model.id, "eligible"); assert.equal(result.eligibleCount, 1);
});

test("tool-capable work excludes models without function calling", () => {
  const result = selectModel({ ...baseIntent, requiresTools: true }, [
    model("fast-text", { supportsTools: false, speed: 1 }), model("agent", { supportsTools: true }),
  ]);
  assert.equal(result.model.id, "agent"); assert.equal(result.eligibleCount, 1);
});
...
test("detail media selects high image detail and ties are deterministic", () => {
  ...
  assert.equal(result.model.id, "alpha"); assert.equal(result.imageDetail, "high"); assert.match(result.reason, /eligible=2/);
});

test("catalog validation rejects duplicate model identifiers", () => {
  assert.throws(() => parseModelCatalog(JSON.stringify([model("same"), model("same")])), /Duplicate model id/);
});
